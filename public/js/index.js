...
const createList = (array, parentElement) => {
  return array.map((segment) => {
    const segmentListItem = listItemWithoutLink(
      segment.number,
      segment.segment
    );
    ...
    if (segment.child && segment.child.length) {
      return segment.child.map((chapter) => {
        const chapterListItem = listItemWithLink(
          chapter.number,
          chapter.chapter
        );
        ...
        chapterListItem.children[1].addEventListener('click', (event) => {
          ...
          document
            ...
            .forEach((rule) => (rule.style.display = 'none'));
          ul.querySelectorAll('.rule').forEach(
            (rule) => (rule.style.display = 'block')
          );
        });
        ...
        chapter.child.map((rule) => {
          ...
          rule.child.map((subrule) => {
            const subruleListItem = listItemWithoutLink(
              subrule.number,
              subrule.subrule
            );
